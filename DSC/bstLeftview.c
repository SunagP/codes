...
int leafnodes(struct node* newnode)
{
  ...
  void leftview(struct node* root, int level, int* hlevel) {
   ...
   leftview(root->left, level + 1, hlevel);
   leftview(root->right, level + 1, hlevel);
}
...
int main()
{
    ...
    while(x)
    {
        ...
        switch(x)
        {
            ...
            case 7 :printf("Number of leaf nodes in the Tree are : %d\n",leafnodes(root));break;
            case 8 : leftview(root, 1, &hlevel);break;
